<script lang="ts">
	...
	let { label, id, name, formName, required, disabled = false }: Props = $props();
...
<div>
	...
	<div class="relative mt-2 rounded-md shadow-sm">
		<textarea
			...
			bind:value={$value}
			class={`block w-full min-h-20 rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6
